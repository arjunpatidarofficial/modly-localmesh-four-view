...
4. Enter full Windows file paths for right, left and back. The Python executable field may be left blank: the adapter checks `LOCALMESH_PYTHON` and common locations such as `C:\\localmesh-engine\\.venv\\Scripts\\python.exe`. If LocalMesh is elsewhere, paste its `.venv\\Scripts\\python.exe` path or set `LOCALMESH_PYTHON`. Some Modly versions open a directory picker for string parameters, so typing/pasting is more reliable.
